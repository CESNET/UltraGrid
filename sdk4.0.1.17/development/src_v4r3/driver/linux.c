...
#define DVS_DRIVERNAME "dvsdriver_v4r3"
...
# ifndef COMPILE_REGPARM
#  define COMPILE_REGPARM
# endif
#endif
...
INTERNAL void linuxmid_set_compatible(int card, int value);
...
INTERNAL void linuxmid_get_addr_status(int card, uintptr * virt, unsigned long * size);
...
INTERNAL void linuxmid_pci_set_ids(int card, int bus, int device, int function, int vendorid, int deviceid);
...
INTERNAL int  linuxmid_pci_probe(uint16 vendor, uint16 device, char ** deviceName, int * cardtype);
...
INTERNAL void DPF_printf(const char * s, ...);
INTERNAL void digilab_gettick(void * handle, uint64 * ptick);
INTERNAL void hugo_gettick(void * handle, uint64 * ptick);
INTERNAL void jpeg_gettick(void * handle, uint64 * ptick);
INTERNAL void hydrax_gettick(void * handle, uint64 * ptick);
...
static int compatible = 0;         /* compatibility mode */
...
int dvs_cardtype_id[PS_CARDTYPE_COUNT];
...
INTERNAL void linux_sort_cards(void)
{
  ...
  for( card = 0; card <= mCards; card ++ ) {
    ...
    DPF_DEBUG("linux_sort_cards() minor:%d -> internal number: %d / serial:%d\n", card, mTranslation[card], dvs_device[mTranslation[card]].serial);
  }
}
...
INTERNAL donttag void * hal_MallocDMATable(void * ps, int size, uintphysaddr * physaddr)
{
  ...
  struct pci_dev * dev = dvs_device[card].dev;
...
INTERNAL donttag void hal_WatchdogTouch(void)
{
...
INTERNAL donttag ps_alenlist * hal_MallocDmaMemory(void * ps, int size, void ** paddr)
{
  int card = linuxmid_get_device_index(ps);
  ...
  if (addr) {
    ...
    if (palenlist) {
      ...
      for (i = 0; i < pages; i++) {
        palenlist->alen[i].physaddr = dma_map_page(
          &device->dev->dev,                            // device
          vmalloc_to_page(addr + (dma_page_size * i)),  // page
          0,                                            // offset
          dma_page_size,                                // size
...
INTERNAL donttag void hal_FreeDmaMemory(ps_rec * ps, ps_alenlist * palenlist, void * addr)
{
  int card = linuxmid_get_device_index(ps);
  dvs_device_t * device = &dvs_device[card];
  int i;
  ...
  for(i = 0; i < palenlist->count; i++) {
    dma_unmap_page(
      &device->dev->dev,            // device
      palenlist->alen[i].physaddr,  // dma_address
      1UL << dmapageshift,          // size
      DMA_BIDIRECTIONAL             // direction
    );
  }
  vfree(addr);
...
INTERNAL donttag static void hal_pages2alenlist(void * ps, ps_alenlist * alenlist, void * _addr, int npages, int bcard2host, int * bhigh)
{
  int card = linuxmid_get_device_index(ps);
  dvs_device_t * device = &dvs_device[card];
  uintptr virttemp; /* temporary virtual address */
  uintptr addr = (uintptr) _addr;
...

  {
    ...
    alenlist->alen[0].physaddr = dma_map_page(
      &device->dev->dev,                            // device
      pages[0],                                     // page
      addr & (PAGE_SIZE-1),                         // offset
      PAGE_SIZE - (addr & (PAGE_SIZE-1)),           // size
      bcard2host ? DMA_FROM_DEVICE : DMA_TO_DEVICE  // direction
    );
    if (PageHighMem(pages[0])) {
      kunmap(pages[0]);
    }
  ...
  for (i = 1; i < npages; i++) {
    {
      ...
      alenlist->alen[i].physaddr = dma_map_page(
        &device->dev->dev,                            // device
        pages[i],                                     // page
        0,                                            // offset
        PAGE_SIZE,                                    // size
...
INTERNAL donttag ps_alenlist * hal_LockMemory(void * ps, int bcard2host, uint64 _addr, uint32 size, int extrapages, int flags, int * error)
{
  void * addr = (void*)(uintptr)_addr;
  ...
  unsigned int dma_page_shift = dmapageshift;
  unsigned int dma_page_size = 1UL << dma_page_shift;
  uint64 dma_page_mask = ~((uint64)dma_page_size-1);

  DPF_DMA_SETUP("hal_LockMemory(ps=%p, bcard2host=%d, addr=%p, size=0x%08x, extrapages=%d, flags=0x%04x)\n", ps, bcard2host, addr, size, extrapages, flags);
  ...
  alenlist->mdl       = mdl;
  alenlist->mdl_pages = npages;

  /*** translate kiovec or pages into alenlist */
  hal_pages2alenlist(ps, alenlist, addr, npages, bcard2host, &is_high);
  ...
  if(PAGE_SIZE != dma_page_size) {
    ...
    alenlist2->mdl_pages = alenlist->mdl_pages;
...
INTERNAL donttag void hal_UnlockMemory(void * ps, ps_alenlist * alenlist)
{
  ...
  if (alenlist) {
    ...
    for (i = 0; i < alenlist->count; i++) {
      dma_unmap_page(
        &device->dev->dev,                                                    // device
        alenlist->alen[i].physaddr,                                           // dma_address
        alenlist->alen[i].size,                                               // size
        alenlist->flags & PS_ALENLIST_READ ? DMA_FROM_DEVICE : DMA_TO_DEVICE  // direction
      );
    }

    nelem = alenlist->mdl_pages;
    pages = alenlist->mdl;
...
INTERNAL donttag void hal_QueueDPC(void * ps, int dpctype)
{
  ...
  if(dpctype & PS_QUEUE_TYPE_CAPTURE) {
    if (waitqueue_active(&device->capture.wait)) {
      atomic_inc(&device->capture.cnt);
      wake_up(&device->capture.wait);
    }
  }
}
...
INTERNAL int hal_CaptureWait(void * ps)
{
  int card = linuxmid_get_device_index(ps);
  dvs_device_t * device = &dvs_device[card];
  int timeout = -1, jiffies = 250 * HZ / 1000; // 250ms
  int res = SV_OK;

  if(atomic_read(&device->capture.cnt) != 0) {
    atomic_set(&device->capture.cnt, 0);
  } else {
    timeout = dvs_wait_event_timeout(device->capture.wait, atomic_read(&device->capture.cnt) > 0, TRUE, jiffies);
    atomic_set(&device->capture.cnt, 0);
  }
...
INTERNAL donttag void hal_GetTick(void * ps, uint64 * ptick)
{
  ...
  case PS_CARDTYPE_IRIS:
    ...
    break;
  case PS_CARDTYPE_DIGILAB:
    digilab_gettick(ps, ptick);
    break;
  case PS_CARDTYPE_JPEG:
    jpeg_gettick(ps, ptick);
    break;
  case PS_CARDTYPE_HYDRAX:
    hydrax_gettick(ps, ptick);
    break;
  default:
    hal_GetTime(ptick);
  }
...
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18)
  struct pci_dev * device = pci_get_bus_and_slot(bus, PCI_DEVFN(dev, function));
#else
...
INTERNAL donttag int hal_GetSystemDate(ps_rec * ps, int * pyear, int * pmonth, int *pday)
{
  struct timeval tv;
  ...
  /* local function for leap year determination */
  int internal_is_leap(int year)
  {
    int leap = 0;
  ...
  /* hours, minutes and seconds don't matter */
  time /= 60;
  time /= 60;
  time /= 24;

  /* count whole years */
  for (year = 1970; time; year++) {
    tmp = internal_is_leap(year) ? 1 : 0;
  ...
  /* count whole months */
  for (month = 1; time && month <= 12; month++) {
    switch (month) {
      case 4: case 6: case 9: case 11:
        tmp = 30;
        break;
      case 2:
        tmp = internal_is_leap(year) ? 29 : 28;
        break;
      default:
        tmp = 31;
        break;
  ...
  /* sanity */
  if (month > 12) {
    DPF_WARN("hal_GetSystemDate: time=%lu day=%d month=%d year=%d\n", time, day, month, year);
...
/**
//  \ingroup linux
//  ioctl that does not use the BKL (Big Kernel Lock, important on SMP systems)
*/
static long dvsdriver_unlocked_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
  int minor = MINOR(file->f_dentry->d_inode->i_rdev);
  int card  = MINOR_TO_INTERNAL( minor );
  int res = SV_OK;

  DBG_IOCTL("%s_unlocked_ioctl: card:%d minor:%d ioctl: cmd:0x%x, arg:0x%x\n", DVS_DRIVERNAME, card, minor, cmd, (int) arg);
  ...
  DBG_IOCTL("%s_unlocked_ioctl: card%d ioctl: cmd:0x%x, arg:0x%x res:0x%x\n", DVS_DRIVERNAME, card, cmd, (int) arg, res);
...
static int dvsdriver_ioctl(struct inode *inode, struct file * file, unsigned int cmd, unsigned long arg)
{
  ...
  DBG_IOCTL("%s_ioctl: card:%d minor:%d ioctl: cmd:0x%x, arg:0x%x\n", DVS_DRIVERNAME, card, minor, cmd, (int) arg);
...
static long dvsdriver_compat_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
  ...
  DBG_IOCTL("dvsdriver_compat_ioctl: card:%d minor:%d ioctl: cmd:0x%x, arg:0x%x\n", card, minor, cmd, (int) arg);
...
static struct page * dvsdriver_vma_nopage(struct vm_area_struct * vma, unsigned long addr, int * type)
{
  ...
  linuxmid_get_addr_status(card, &virtaddr, &max_offset);
  ...
  if(offset + PAGE_SIZE > max_offset) {
    DBG_MMAP("%s_vma_nopage: offset too high (0x%x)\n", DVS_DRIVERNAME, (unsigned int)max_offset);
    ok = FALSE;
  }
...
static int dvsdriver_vma_fault(struct vm_area_struct * vma, struct vm_fault *vmf)
{
  ...
  DBG_MMAP("%s_vma_nopage(vma=%p,addr=%08x) card:%d offset:%08x max_offset:%08x\n", DVS_DRIVERNAME, vma, addr, /*type,*/ card, offset, max_offset);
  ...
  if(result == 0) {
    if(down_killable(&device->vma_mutex)) {
      result = VM_FAULT_SIGBUS;
    }
...
static int dvsdriver_mmap(struct file * file, struct vm_area_struct * vma)
{
  ...
  if (offset > max_offset) {
    DBG_MMAP("%s_mmap: offset too high (0x%x)\n", DVS_DRIVERNAME, (unsigned int)max_offset);
    return -EINVAL;
  }
  if (offset + size > max_offset) {
    DBG_MMAP("%s_mmap: offset+size too high (0x%x)\n", DVS_DRIVERNAME, (unsigned int)max_offset);
    return -EINVAL;
  }
  ...
  DBG_MMAP("%s_mmap: mapped area %d physical address %p\n", DVS_DRIVERNAME, area, vm_offset);
...
static int dvsdriver_open(struct inode * inode, struct file * file)
{
  ...
  if(card >= DVS_MAXDEVICES) {
    return -ENODEV;
  }

  if(!(dvs_device[card].mask & DVSDEVICE_EXISTS)) {
    return -ENODEV;
  }
...
#if HAVE_UNLOCKED_IOCTL
  .unlocked_ioctl = dvsdriver_unlocked_ioctl,
#else
...
static struct pci_device_id dvs_pci_tbl[] = {
  {PCI_DEVICE(PCI_VENDORID_DVS   , PCI_DEVICEID_DVS_ATOMIX   )},
  {PCI_DEVICE(PCI_VENDORID_DVS   , PCI_DEVICEID_DVS_ATOMIXLT )},
  {0,}
};
...
int init_module(void)
{
  ...
  memset(dvs_cardtype_id, 0, sizeof(int) * arraysize(dvs_cardtype_id));

  kobject_set_name(&dvsdriver_cdev.kobj, DVS_DRIVERNAME);
...
int linux_map_memory( int card, struct pci_dev *pDev )
{
  ...
  for (i = 0; i < linuxmid_pci_maxbadr(card); i++) {
    ...
    linuxmid_pci_limit_mapping(card, memoryarea, &mapped);
    ...
    if (physaddr) {
      ...
      linuxmid_pci_set_badr(card, memoryarea, busaddr, physaddr, length, mapped, virtaddr);
...
void linux_unmap_memory( int card, struct pci_dev *pDev )
{
  ...
  for (i = 0; i < linuxmid_pci_maxbadr(card); i++) {
    ...
    linuxmid_pci_set_badr(card, i, 0, 0, 0, 0, (uintptr)NULL);
  }
}
...
void init_dvs_device_struct( int card, struct pci_dev *pDev, char *name, int cardtype )
{
  int cardtype_id = dvs_cardtype_id[cardtype]++;
  int jack;
  ...
  sprintf( dvs_device[card].name, name, cardtype_id );
  ...
  init_waitqueue_head(&dvs_device[card].capture.wait);
  init_waitqueue_head(&dvs_device[card].userdma);
  atomic_set(&dvs_device[card].userdma_cnt, 1);
#ifndef init_MUTEX
  sema_init(&dvs_device[card].vma_mutex, 1);
#else
...
void init_midlayer( int card, struct pci_dev *pDev, int cardtype )
{
  ...
  linuxmid_set_compatible( card, compatible );
  ...
  linuxmid_pci_set_ids( card, pDev->bus->number, PCI_SLOT(pDev->devfn), PCI_FUNC(pDev->devfn), pDev->vendor, pDev->device );
}
...
int dvsdriver_pci_probe( struct pci_dev *pDev, const struct pci_device_id *pId )
{
  ...
  if( result == 0 ) {
    result = linuxmid_pci_probe(pDev->vendor, pDev->device, &deviceName, &cardtype);
  }
  ...
  if( result == 0 ) {
    init_dvs_device_struct( card, pDev, deviceName, cardtype ); //Need free_dvs_device_struct()
  }
