...
#define DVS_DRIVERNAME "dvsdriver_lucy"
...
INTERNAL void vtrslave_odetics_dpc(void * handle, void * vtr);
...
INTERNAL donttag ps_alenlist * hal_LockMemory(void * ps, int bcard2host, uint64 _addr, uint32 size, int extrapages, int * error)
{
...
INTERNAL donttag void hal_QueueOdetics(void * ps, void * vtr)
{
  vtrslave_odetics_dpc(ps, vtr);
}
  ...
  *pcount = 0;
  ...
  if( ret == SV_OK )
  {
    ...
    if (file == NULL || IS_ERR(file))
    {
      ...
      if (strcmp(filename, "userdef_lucy.ref") != 0) {
        // loading of userdef_lucy.ref may fail, because not always present
...
int dvsdriver_pci_probe( struct pci_dev *pDev, const struct pci_device_id *pId )
{
  ...
  case PCI_VENDORID_XILINX:
    ...
    case PCI_DEVICEID_XILINX_IRIS:
      deviceName = "iris%d"; // We will not change the deviceName into lucy
