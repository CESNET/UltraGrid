...
#define DVS_DRIVERNAME "dvsdriver_sdio"
...
#define DMA_PAGE_SHIFT 12
#define DMA_PAGE_SIZE (1UL << DMA_PAGE_SHIFT)
#define DMA_PAGE_MASK (~(DMA_PAGE_SIZE - 1))

/* Consistent allocation has problems with memory fragementation.
   Do not use it any more. - To be removed later. */
#ifdef DVS_BOUNCE_CONSISTENT
# define DVS_BOUNCE_VIRT(address) (device->bounce_virt + (address))
# define DVS_BOUNCE_BUS(address) (device->bounce_phys + (address))
#else
# define DVS_BOUNCE_MAX 131072    // biggest size in slab allocator
# define DVS_BOUNCE_VIRT(address) (device->bounce_virt[(address) / DVS_BOUNCE_MAX] + (address) % DVS_BOUNCE_MAX)
# define DVS_BOUNCE_BUS(address) (hal_GetBusAddress(ps, (void *)DVS_BOUNCE_VIRT(address)))
#endif
...
typedef struct {
  ...
  wait_queue_head_t vsyncwait_rec;
  wait_queue_head_t vsyncwait_dis;
  wait_queue_head_t fifowait_rec;
  wait_queue_head_t fifowait_dis;

  int               vsyncwait_rec_res;
  int               vsyncwait_dis_res;
  atomic_t          vsyncwait_rec_cnt;
  atomic_t          vsyncwait_rec_wakeup;
  atomic_t          vsyncwait_dis_cnt;
  atomic_t          vsyncwait_dis_wakeup;

  atomic_t          fifowait_rec_cnt;
  atomic_t          fifowait_dis_cnt;
...
INTERNAL int  linuxmid_debug_getmask(int card);
INTERNAL void linuxmid_debug_setmask(int card, int mask);
INTERNAL int  linuxmid_debug_parse(int card, const char * word, int * mask);
INTERNAL void linuxmid_debug_psconf(int card, const char * node);
INTERNAL void DPF_printf(char * s, ...);
INTERNAL void sdio_gettick(void * handle, uint64 * ptick);
...
static int dobounce = 1;                /* perform bounce copy if needed */
#endif
...
INTERNAL donttag static void hal_pages2alenlist(void * ps, ps_alenlist * alenlist, int npages, int bcard2host, int * bhigh)
{
  ...
  if (do_bounce) {
    alenlist->alen[0].physaddr = DVS_BOUNCE_BUS(addr & (PAGE_SIZE-1));
    virttemp = DVS_BOUNCE_VIRT(addr & (PAGE_SIZE-1));
  } else {
  ...
  //DPF_DMA2("hal_pages2alenlist: i=%d virt=%p phys=0x%08x%08x size=0x%x\n", 0, virttemp, (uint32)(alenlist->alen[0].physaddr>>32), (uint32)alenlist->alen[0].physaddr, alenlist->alen[0].size);

  for (i = 1; i < npages; i++) {
    if (do_bounce) {
      alenlist->alen[i].physaddr = DVS_BOUNCE_BUS(PAGE_SIZE * i);
      virttemp = DVS_BOUNCE_VIRT(PAGE_SIZE * i);
    } else {
...
INTERNAL donttag ps_alenlist * hal_LockMemory(void * ps, int bcard2host, void * addr, uint32 size, int extrapages, int * error)
{
  ...
  DPF_DMA("hal_LockMemory(ps=%p, bcard2host=%d, addr=%p, size=0x%08x, extrapages=%d)\n", ps, bcard2host, addr, size, extrapages);
  ...
  if (do_bounce) {
    /*** bounce buffer copy for PAE */
    /* get mutex first as we only have one bounce buffer */
    down(&device->bounce_mutex);

    device->bounce_size = size + PAGE_SIZE;
#ifdef DVS_BOUNCE_CONSISTENT
    device->bounce_virt = (uintptr)pci_alloc_consistent(device->dev, device->bounce_size, &device->bounce_phys);
#else
    device->bounce_items  = device->bounce_size / DVS_BOUNCE_MAX;
    device->bounce_items += device->bounce_size % DVS_BOUNCE_MAX ? 1 : 0;

    device->bounce_virt = kmalloc(sizeof(uintptr) * device->bounce_items, GFP_KERNEL);

    if (device->bounce_virt) {
      for (i = 0; i < device->bounce_items; i++) {
        device->bounce_virt[i] = (uintptr)kmalloc(DVS_BOUNCE_MAX, GFP_KERNEL | GFP_DMA);
        if (!device->bounce_virt[i]) {
          break;
        }
      }
      if (i < device->bounce_items) {
        DPF_WARN("hal_LockMemory: bounce buffer allocation failed (%dof%d)\n", i, device->bounce_items);

        // rollback in case allocating a single item failed
        for ( ; i > 0; i--) {
          kfree((void *)device->bounce_virt[i - 1]);
        }
        kfree(device->bounce_virt);
        device->bounce_virt = NULL;
      }
    }
#endif
    if(!device->bounce_virt) {
      up(&device->bounce_mutex);
      DPF_WARN("hal_LockMemory: bounce buffer allocation failed\n");
      errorcode = SV_ERROR_MALLOC_FRAGMENTED;
      goto out;
    }
  }
  ...
  DPF_DMA("hal_LockMemory: in:%p out:%p npages=%d totalpages=%d\n", in, out, npages, totalpages);
...
#if PAGE_SIZE != DMA_PAGE_SIZE
  {
    ...
    DPF_DMA("hal_LockMemory: dma_in:%p dma_out:%p dma_npages=%d dma_totalpages=%d\n", dma_in, dma_out, dma_npages, dma_totalpages);
  ...
  if (do_bounce) {
    /* bounce data into low memory - only for write access */
    if (!bcard2host) {
      unsigned int offset = (uintptr) alenlist->addr & (PAGE_SIZE - 1);
      unsigned int nbytes = alenlist->size;
      uintptr ptr         = (uintptr) alenlist->addr;
      uintptr virttemp    = DVS_BOUNCE_VIRT(offset);
#ifdef DVS_BOUNCE_CONSISTENT
      dummy = copy_from_user((void *) virttemp, (const void *) ptr, nbytes);
#else
      for (i = 0; i < device->bounce_items; i++) {
        unsigned int count = nbytes < DVS_BOUNCE_MAX ? nbytes : DVS_BOUNCE_MAX;

        if (i == 0) {
          virttemp = DVS_BOUNCE_VIRT(offset);
          if (count + offset > DVS_BOUNCE_MAX) {
            count -= (count + offset) - DVS_BOUNCE_MAX;
          }
        } else {
          virttemp = DVS_BOUNCE_VIRT(i * DVS_BOUNCE_MAX);
        }

        dummy = copy_from_user((void *) virttemp, (const void *) ptr, count);
    ...
    switch (linuxmid_get_cardtype(linuxmid_get_device_index(ps))) {
    case PS_CARDTYPE_SDIO:
      printk("%s: high memory not allowed for DMA transfer\n", DVS_DRIVERNAME );
      DPF_WARN("hal_LockMemory: bounce buffer disabled - skipping DMA\n");
      errorcode = SV_ERROR_HIGH_MEMORY;
      goto out_free_pages;
    }
  }
...
out_free_bounce:
  if (do_bounce) {
#ifdef DVS_BOUNCE_CONSISTENT
    pci_free_consistent(device->dev, device->bounce_size, (char *)device->bounce_virt, device->bounce_phys);
#else
    for ( ; device->bounce_items > 0; device->bounce_items--) {
      kfree((void *)device->bounce_virt[device->bounce_items - 1]);
    }
    kfree(device->bounce_virt);
#endif
    up(&device->bounce_mutex);
...
INTERNAL donttag void hal_UnlockMemory(void * ps, ps_alenlist * alenlist)
{
  ...
  if (alenlist) {
    ...
    if (do_bounce) {
      ...
      /* bounce data back into high memory - only for read access */
      if (alenlist->flags & PS_ALENLIST_READ) {
        ...
        uintptr virttemp    = DVS_BOUNCE_VIRT(offset);
#ifdef DVS_BOUNCE_CONSISTENT
        dummy = copy_to_user((void *) ptr, (const void *) virttemp, nbytes);
#else
        for (i = 0; i < device->bounce_items; i++) {
          unsigned int count = nbytes < DVS_BOUNCE_MAX ? nbytes : DVS_BOUNCE_MAX;
          ...
          dummy = copy_to_user((void *) ptr, (const void *) virttemp, count);
...
INTERNAL donttag void hal_QueueDPC(void * ps, int dpctype)
{
  ...
  switch (dpctype) {
  case PS_QUEUE_TYPE_VSYNC_RECORD:
    // wake all, that are sleeping at the moment
    device->vsyncwait_rec_res = 1;
    for(count = atomic_read(&device->vsyncwait_rec_cnt); count > 0; count--) {
      if (waitqueue_active(&device->vsyncwait_rec)) {
        atomic_set(&device->vsyncwait_rec_wakeup, atomic_read(&device->vsyncwait_rec_cnt));
        wake_up(&device->vsyncwait_rec);
      }
    }
    break;
  case PS_QUEUE_TYPE_VSYNC_DISPLAY:
    // wake all, that are sleeping at the moment
    device->vsyncwait_dis_res = 1;
    for(count = atomic_read(&device->vsyncwait_dis_cnt); count > 0; count--) {
      if (waitqueue_active(&device->vsyncwait_dis)) {
        atomic_set(&device->vsyncwait_dis_wakeup, atomic_read(&device->vsyncwait_dis_cnt));
        wake_up(&device->vsyncwait_dis);
      }
    }
    break;
  case PS_QUEUE_TYPE_AUDIO_RECORD:
  case PS_QUEUE_TYPE_AUDIO_DISPLAY:
    break;
  case PS_QUEUE_TYPE_FIFO_RECORD:
    // wake one, in case there is one
    if (atomic_read(&device->fifowait_rec_cnt) > 0) {
      atomic_dec(&device->fifowait_rec_cnt);
      if (waitqueue_active(&device->fifowait_rec)) {
        wake_up(&device->fifowait_rec);
      }
    }
    break;
  case PS_QUEUE_TYPE_FIFO_DISPLAY:
    // wake one, in case there is one
    if (atomic_read(&device->fifowait_dis_cnt) > 0) {
      atomic_dec(&device->fifowait_dis_cnt);
      if (waitqueue_active(&device->fifowait_dis)) {
        wake_up(&device->fifowait_dis);
      }
    }
    break;
  default:
...
INTERNAL int hal_VSyncWaitCancel(void * ps)
{
  int card = linuxmid_get_device_index(ps);
  dvs_device_t * device = &dvs_device[card];
  int rec, dis;

  rec = (int)waitqueue_active(&device->vsyncwait_rec);
  dis = (int)waitqueue_active(&device->vsyncwait_dis);

  if (dis || rec) {
    /* wake up all processes waiting for a vsync */
    if(rec) {
      atomic_set(&device->vsyncwait_rec_wakeup, atomic_read(&device->vsyncwait_rec_cnt));
      device->vsyncwait_rec_res = 2;
      wake_up_all(&device->vsyncwait_rec);
    }
    if(dis) {
      atomic_set(&device->vsyncwait_dis_wakeup, atomic_read(&device->vsyncwait_dis_cnt));
      device->vsyncwait_dis_res = 2;
      wake_up_all(&device->vsyncwait_dis);
    }
  }
...
INTERNAL int hal_VSyncWait(void * ps, int jack)
{
  ...
  if (jack == JACK_INPUT) {
    atomic_inc(&device->vsyncwait_rec_cnt);

    timeout = dvs_wait_event_timeout(device->vsyncwait_rec, atomic_read(&device->vsyncwait_rec_wakeup) > 0, FALSE, jiffies);
    ret = device->vsyncwait_rec_res;

    if (atomic_read(&device->vsyncwait_rec_wakeup) > 0) {
      atomic_dec(&device->vsyncwait_rec_wakeup);
    }

    if (atomic_read(&device->vsyncwait_rec_cnt) > 0) {
      atomic_dec(&device->vsyncwait_rec_cnt);
    } else {
      device->vsyncwait_rec_res = 0;
    }
  } else {
    atomic_inc(&device->vsyncwait_dis_cnt);

    timeout = dvs_wait_event_timeout(device->vsyncwait_dis, atomic_read(&device->vsyncwait_dis_wakeup) > 0, FALSE, jiffies);
    ret = device->vsyncwait_dis_res;

    if (atomic_read(&device->vsyncwait_dis_wakeup) > 0) {
      atomic_dec(&device->vsyncwait_dis_wakeup);
    }

    if (atomic_read(&device->vsyncwait_dis_cnt) > 0) {
      atomic_dec(&device->vsyncwait_dis_cnt);
    } else {
      device->vsyncwait_dis_res = 0;
    }
  }
...
INTERNAL int hal_FifoWait(void * ps, int jack, ps_lock * plock)
{
  ...
  if(jack == JACK_INPUT) {
    atomic_inc(&device->fifowait_rec_cnt);

    if(plock) {
      hal_Unlock(ps, plock);
    }

    DBG_HAL("hal_FifoWait: called for record\n");

    timeout = dvs_wait_event_timeout(device->fifowait_rec, atomic_read(&device->fifowait_rec_cnt) <= 0, TRUE, jiffies);
    ...
    atomic_inc(&device->fifowait_dis_cnt);

    if(plock) {
      hal_Unlock(ps, plock);
    }

    DBG_HAL("hal_FifoWait: called for display\n");

    timeout = dvs_wait_event_timeout(device->fifowait_dis, atomic_read(&device->fifowait_dis_cnt) <= 0, TRUE, jiffies);
...
#ifdef COMPILE_SDIO
  {PCI_DEVICE(PCI_VENDORID_XILINX, PCI_DEVICEID_XILINX_SDIO  )},
  {PCI_DEVICE(PCI_VENDORID_DVS   , PCI_DEVICEID_DVS_SDIO     )},
  {PCI_DEVICE(PCI_VENDORID_DVS   , PCI_DEVICEID_DVS_SDIO_EDDY)},
#endif
#ifdef COMPILE_IRIS
...
void init_dvs_device_struct( int card, struct pci_dev *pDev, char *name )
{
  ...
  init_waitqueue_head(&dvs_device[card].vsyncwait_rec);
  init_waitqueue_head(&dvs_device[card].vsyncwait_dis);
  init_waitqueue_head(&dvs_device[card].fifowait_rec);
  init_waitqueue_head(&dvs_device[card].fifowait_dis);
  atomic_set(&dvs_device[card].fifowait_rec_cnt, 0);
  atomic_set(&dvs_device[card].fifowait_dis_cnt, 0);
  dvs_device[card].vsyncwait_rec_res = 0;
  dvs_device[card].vsyncwait_dis_res = 0;
  atomic_set(&dvs_device[card].vsyncwait_rec_cnt, 0);
  atomic_set(&dvs_device[card].vsyncwait_dis_cnt, 0);
  atomic_set(&dvs_device[card].vsyncwait_rec_wakeup, 0);
  atomic_set(&dvs_device[card].vsyncwait_dis_wakeup, 0);
  init_MUTEX(&dvs_device[card].bounce_mutex);
...
int dvsdriver_pci_probe( struct pci_dev *pDev, const struct pci_device_id *pId )
{
  ...
  case PCI_VENDORID_XILINX:
    switch(pDev->device)
    {
    case PCI_DEVICEID_XILINX_SDIO:
      deviceName = "sdboard%d";
      cardtype   = PS_CARDTYPE_SDIO;
      break;
...
#ifdef CONFIG_HIGHMEM64G
  if( result == 0 ) {
      switch (linuxmid_get_cardtype(card)) {
        case PS_CARDTYPE_SDIO:
          if( dobounce ) {
             printk("%s: CONFIG_HIGHMEM64G is enabled - using bounce buffers for card %d\n", DVS_DRIVERNAME, card);
          }
          break;
      }
  }
...
donttag int dvs_debug_write_proc(struct file *file, const char *buf, unsigned long len, void *data)
{
  ...
  tmp = linuxmid_debug_getmask(0);

  if (!linuxmid_debug_parse(0, word, &tmp)) {
    if (strcmp(word, "NOUSE") == 0) {       /* reset usage counter */
      printk("Modules use counter reset - not available on this kernel.\n");
#ifdef DEBUG
      printk("Driver compiled with CONFIG_MODULE_FORCE_UNLOAD. Use 'rmmod -f'.\n");
#endif
    } else if(strcmp(word, "help") == 0) {
      printk(
        "dvsdriver: possible words are:\n"
        "  audio audio2 audioirq bios cmd ctrl ctrl2 dma dma2\n"
        "  dram dram2 fifo fifo2 header init irq irq2 master master2 rgbm queue sched\n"
        "  sched2 svhs video video2 vtrslave vtrmaster videoirq off\n"
        "  One word at a time. Words are ORed\n"
        "  off - clear all debug flags\n"
        "  NOUSE - reset module usage counter\n"
      );
    } else {
      printk("%s: unknown word\n", DVS_DRIVERNAME );
    }
  }

  linuxmid_debug_setmask(0, tmp);

  printk("debug flag now is 0x%x\n", tmp);      /* give feedback */
...
INTERNAL donttag int hal_InitBounceBuffer( ps_rec * ps )
{
  return SV_OK;
}

INTERNAL donttag void hal_FreeBounceBuffer( ps_rec * ps )
{
}
