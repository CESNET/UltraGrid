  ...
  /*** field animation ************************************************/
    ...
    for (alpha=s->startangle, frame=start; frame<start+limit; frame++)
    {
      ...
      for (field=1; field<=2; field++, alpha+=half_angle) {
        if ((s->field==SEQUENCE_FIRST_ODD  && field==1) ||
            (s->field==SEQUENCE_FIRST_EVEN && field==1))
          alpha+=half_angle;
        else if
          ((s->field==SEQUENCE_FIRST_ODD  && field==2) ||
           (s->field==SEQUENCE_FIRST_EVEN && field==2))
          alpha-=s->angle;
