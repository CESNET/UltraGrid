...
void jpeg_mode(sv_handle * sv, char * videomode)
{
  ...
  if((videomode[0] >= '0') && (videomode[0] <= '9')) {
    rasterindex = TRUE;
    video       = atoi(videomode) | SV_MODE_FLAG_RASTERINDEX;
  }
...
static char *str_outduringrec[] = { "default", "input", "output", "black", "bypass", "colorbar" };
...
static char *str_overlay[] = { "off", "on", "transparent"};

static char *str_overlay_type_pv[] = { "none", "ltc_timecode", "ltc_frame_abs",
                                       "ltc_frame_rel", "timecode", 
                                       "frame_abs", "frame_rel",
                                       "ltcinput_timecode" };
...
int jpeg_support_videomode2string(sv_handle * sv, char * buffer, int mode)
{
  ...
  if((mode & SV_MODE_ACTIVE_MASK) == SV_MODE_ACTIVE_STREAMER) {
    strcat(buffer, "/RAW");
  } 
...
void jpeg_info(sv_handle * sv)
{
  ...
  if(res == SV_OK) {
    ...
    printf("Video sync      : %s %s\n", buffer, syncstate?"":"(Sync Missing)");
...
void jpeg_licence(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc > 1) {
    if(!strcmp(argv[1], "help") || !strcmp(argv[1], "?")) {
      ...
      printf("\tkey1 - Set key1 for SDBoard/HDBoard/Centaurus\n");
      printf("\tkey2 - Set key1 for SDBoard/HDBoard/Centaurus\n");
      printf("\tkey3 - Set key1 for SDBoard/HDBoard/Centaurus\n");
      printf("\tkeyfile1 - Set key1 from file\n");
      printf("\tkeyfile2 - Set key2 from file\n");
      return;
    }
  }
...
void jpeg_iomode(sv_handle * sv, char * what, char * param2)
{
  ...
  if(!strcmp(what, "info")) {
    ...
    printf("\tYUV422\tSets IO mode to YUV422\n");
    printf("\tYUV444\tSets IO mode to YUV444\n");
    printf("\tRGB\tSets IO mode to RGB\n");
    printf("\tYUV422A\tSets IO mode to YUV422A\n");
    printf("\tYUV444A\tSets IO mode to YUV444A\n");
    printf("\tRGBA\tSets IO mode to RGBA\n");
...
void jpeg_overlay(sv_handle * sv, int argc, char ** argv)
{
  int res = SV_OK;
  int cmd = SV_OPTION_NOP;
  int par = 0;

  if (argc == 1) {
    cmd = SV_OPTION_OVERLAY_MODE;
    if((strcmp(argv[0], "on") == 0) || (strcmp(argv[0], "analog") == 0)) {
      par = SV_OVERLAY_ON;
    } else if (strcmp(argv[0], "digital") == 0) {
      par = SV_OVERLAY_ON | SV_OVERLAY_DIGITAL;
    } else if (strcmp(argv[0], "off") == 0) {
      par = SV_OVERLAY_OFF;
    } else if (!strcmp(argv[0], "transparent")) {
      par = SV_OVERLAY_TRANSPARENT;
    } else if(!strcmp(argv[0], "info")) {
      res = sv_query(sv, SV_QUERY_OVERLAYMODE, 0, &par);
      if(res == SV_OK) {
        if((par >= 0) && (par < arraysize(str_overlay))) {
          printf("sv overlay %s\n", str_overlay[par]);
        }
      }
      return;
      ...
      printf("\ttransparent\n");
      printf("\tclear\n");
      printf("\twrite xpos ypos text\n");
      printf("\tinfo\n");
      printf("\tentry xpos ypos xsize ysize [transparency] text\n");
      return;
    } else {
    ...
    if(strcmp(argv[0], "type") == 0) {
      if(!strcmp(argv[1], "info")) {
        res = sv_query(sv, SV_QUERY_OVERLAYTYPE, 0, &par);
        if(res == SV_OK) {
          if((par >= 0) && (par < arraysize(str_overlay_type_pv))) {
            printf("sv overlay type %s\n", str_overlay_type_pv[par]);
          }
        }
        return;
      } else {
        for (par = 0; par < arraysize(str_overlay_type_pv); par++) {
          if (strcmp(argv[1], str_overlay_type_pv[par]) == 0) {
            cmd = SV_OPTION_OVERLAY_TYPE;
            break;
          }
        }
        if (par == arraysize(str_overlay_type_pv)) {
          jpeg_errorprint(sv, SV_ERROR_PARAMETER);
          return;
        }
      }
    } else if (strcmp(argv[0], "frameoffset") == 0) {
      if(!strcmp(argv[1], "info")) {
        res = sv_query(sv, SV_QUERY_OVERLAY_FRAMEOFFSET, 0, &par);
        if(res == SV_OK) {
            printf("sv overlay frameoffset %d\n", par);
        }
        return;
      } else {
        cmd = SV_OPTION_OVERLAY_FRAMEOFFSET;
        par = atoi(argv[1]);
      }
    } else {
      jpeg_errorprint(sv, SV_ERROR_PARAMETER);
    ...
    if(strcmp(argv[0], "write") == 0) {
      res = sv_overlaytext(sv, atoi(argv[1]), atoi(argv[2]), argv[3], (int)strlen(argv[3]));
    } else {
      res = SV_ERROR_PARAMETER;
    }
  } else if(argc >= 7) {
    if(strcmp(argv[0], "entry") == 0) {
      int textcolor = 0x10000;
      int backcolor = 0x00000;
      int flags     = 0;

      if(argc > 7) {
        textcolor = (int)(0x10000 * atof(argv[6]));
      }
      if(argc > 8) {
        backcolor = (int)(0x10000 * atof(argv[7]));
      }
      if(argc > 9) {
        flags = atoi(argv[8]);
      }
      res = sv_overlay(sv, atoi(argv[1]), (int)(0x10000 * atof(argv[2])), (int)(0x10000 * atof(argv[3])), atoi(argv[4]), atoi(argv[5]), textcolor, backcolor, flags, argv[argc-1]);
    } else {
      res = SV_ERROR_PARAMETER;
    }
  } else {
...
void jpeg_gamma(sv_handle * sv, int argc, char ** argv)
{
  int res    = SV_OK;
  int next   = -1;
  int option = 0;
  ...
  if(argc >= 1) {
    if(isdigit((unsigned char)argv[0][0])) {
      option = SV_OPTION_GAMMA;
      tmp    = (int) (0x10000 * atof(argv[0]));
      next = 1;
    } else if(!strcmp(argv[0], "rgb")) {
      if(argc >= 4) {
        option = -1;
        red    = (int) (0x10000 * atof(argv[1]));
        grn    = (int) (0x10000 * atof(argv[2]));
        blu    = (int) (0x10000 * atof(argv[3]));
      } else {
        res = SV_ERROR_PARAMETER;
      }
      next = 4;
      ...
      option = SV_OPTION_GAMMA;
      tmp    = 0;
    } else if(!strcmp(argv[0], "lut")) {
      res = sv_option_set(sv, SV_OPTION_GAMMA_LUT, 1);
      option = 0;
    } else if(!strcmp(argv[0],"save")) {
      res = sv_query(sv, SV_QUERY_GAMMA, 0, &tmp);
      if((res == SV_OK) && (argc >= 2)) {
        if(strcmp(argv[1],"default")==0) {
          sv_option_save(sv, SV_OPTION_GAMMA, 0);
        } else if(strcmp(argv[1],"userdef")==0) {
          switch(tmp & SV_GAMMA_MODE_MASK) {
          case SV_GAMMA_MODE_LUMA:
          default:
            sv_option_get(sv,  SV_OPTION_GAMMA, &tmp);
            sv_option_save(sv, SV_OPTION_GAMMA, tmp);
            break;
          case SV_GAMMA_MODE_RGB:
            sv_option_get(sv,  SV_OPTION_GAMMA_RED, &tmp);
            sv_option_save(sv, SV_OPTION_GAMMA_RED, tmp);
            sv_option_get(sv,  SV_OPTION_GAMMA_GREEN, &tmp);
            sv_option_save(sv, SV_OPTION_GAMMA_GREEN, tmp);
            sv_option_get(sv,  SV_OPTION_GAMMA_BLUE, &tmp);
            sv_option_save(sv, SV_OPTION_GAMMA_BLUE, tmp);
            break;
          }
        } else {
          res = SV_ERROR_PARAMETER;
      ...
      if(res == SV_OK) {
        switch(tmp & SV_GAMMA_MODE_MASK) {
        case SV_GAMMA_MODE_DISABLED:
          printf("sv gamma off");
          break;
        case SV_GAMMA_MODE_LUMA:
          res = sv_query(sv, SV_QUERY_GAMMA, 0, &tmp);
          printf("sv gamma %1.3f", ((double)(tmp & SV_GAMMA_VALUE_MASK) / 0x10000));
          break;
        case SV_GAMMA_MODE_RGB:
          res = sv_query(sv, SV_QUERY_GAMMA_RED, 0, &red); red &= SV_GAMMA_VALUE_MASK;
          if(res == SV_OK) {
            res = sv_query(sv, SV_QUERY_GAMMA_GREEN, 0, &grn); grn &= SV_GAMMA_VALUE_MASK;
          }
          if(res == SV_OK) {
            res = sv_query(sv, SV_QUERY_GAMMA_BLUE, 0, &blu); blu &= SV_GAMMA_VALUE_MASK;
          }
          printf("sv gamma rgb %1.3f %1.3f %1.3f", ((double)red / 0x10000), ((double)grn / 0x10000), ((double) blu/ 0x10000));
          break;
        case SV_GAMMA_MODE_LUT:
          printf("sv gamma lut\n");
          break;
        default:
          printf("sv gamma ?\n");
        ...
        if((tmp & SV_GAMMA_MODE_MASK) != SV_GAMMA_MODE_DISABLED) {
          if(tmp & SV_GAMMA_FULLRANGE) {
            printf(" fullrange");
          }
          if(tmp & SV_GAMMA_ANALOG) {
            printf(" analog");
          }
          if(tmp & SV_GAMMA_DIGITAL) {
            printf(" digital");
          }
          if(tmp & SV_GAMMA_8BIT) {
            printf(" 8bit");
          }
      ...
      printf("sv gamma help\n");
      printf("\tvalue\tSet luma Gamma\n");
      printf("\tred value\n");
      printf("\tgreen value\n");
      printf("\tblue value\n");
      printf("\trgb value\n");
  ...
  if((res == SV_OK) && (next != -1)) {
    if(argc > next) {
      while(argc > next) {
        if(!strcmp(argv[next], "fullrange")) {
          mask |= SV_GAMMA_FULLRANGE;
        } else if(!strcmp(argv[next], "analog")) {
          mask |= SV_GAMMA_ANALOG;
        } else if(!strcmp(argv[next], "digital")) {
          mask |= SV_GAMMA_DIGITAL;
        } else if(!strcmp(argv[next], "8bit")) {
          mask |= SV_GAMMA_8BIT;
        } else {
          res = SV_ERROR_PARAMETER;
        }
  ...
  if(res == SV_OK) {
    if(!(mask & (SV_GAMMA_ANALOG|SV_GAMMA_DIGITAL))) {
      mask |= SV_GAMMA_ANALOG|SV_GAMMA_DIGITAL;
    }
    ...
    case 0:
      break;
    case -1:
      res = sv_option(sv, SV_OPTION_GAMMA_RED, mask|red);
      if(res == SV_OK) {
        res = sv_option(sv, SV_OPTION_GAMMA_GREEN, mask|grn);
      }
      if(res == SV_OK) {
        res = sv_option(sv, SV_OPTION_GAMMA_BLUE, mask|blu);
      }
      break;
    default:
      res = sv_option(sv, option, mask|tmp);
    }
  }
...
void jpeg_lut(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 1) {
    if(!strcmp(argv[0], "luma")) {
      ...
      printf("sv lut {luma,red,green,blue,disable} file.lut\n");
      printf("\tFile format:\n");
      printf("\t#LUT{/I=interpolate}\n");
      printf("\t0        value\n");
      printf("\tposition value\n");
      printf("\t1023     value\n");
      printf("\n");
      printf("\tIf not /I is used then the file must contain 1024 values.\n");
    ...
    if(res == SV_OK) {
      for(lastvalue = pos = 0; pos < 1024;) {
        ...
        if(!interpolate) {
          ...
          lut[pos++] = value;
          for(orgpos = pos-1; (pos <= tmp) && (pos < 1024); pos++) {
            lut[pos] = lastvalue + ((pos - orgpos) * (4096 * (value - lastvalue)) / (tmp - orgpos)) / 4096;
          }
        }
        count = jpeg_read_line(path, buffer);
        lastvalue = value;
...
void jpeg_sync(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(syncmode == -1) {
    if(!strcmp(argv[0], "hdelay") && (argc > 1)) {
      ...
      printf("\texternal\tSync to incoming SDI signal\n");
      ...
      printf("\tvdelay #\tVertical sync delayn\n");
...
int jpeg_rs422pinout(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 1) {
    if(!strcmp(argv[0], "help")) {
      ...
      printf("\tporta <pinout> <task>\n");
      printf("\tportb <pinout> <task>\n");
      printf("\tportc <pinout> <task>\n");
      printf("\tportd <pinout> <task>\n");
      printf("\t\t<pinout>  default,normal,swapped\n");
      printf("\t\t<task>    default,none,master,slave\n");
    } else if(!strcmp(argv[0], "info")) {
      printf("         %-5s %-7s %-5s\n", "port", "pinout", "task");
      for(device = 0; device < 4; device++) {
        if(device == 0) {
          res = sv_option_get(sv, SV_OPTION_RS422A_PINOUT, &value);
        } else if(device == 1) {
          res = sv_option_get(sv, SV_OPTION_RS422B_PINOUT, &value);
        } else if(device == 2) {
          res = sv_option_get(sv, SV_OPTION_RS422C_PINOUT, &value);
        } else if(device == 3) {
          res = sv_option_get(sv, SV_OPTION_RS422D_PINOUT, &value);
        }

        if(res == SV_OK) {
          pinout = value & SV_RS422_PINOUT_MASK;
          task = value & SV_RS422_TASK_MASK;
          printf("sv rs422pinout port%c %-7s %-5s\n", device + 'a',
            (pinout == SV_RS422_PINOUT_NORMAL)  ? "normal" : "swapped",
            (task == SV_RS422_TASK_MASTER) ? "master" : (task == SV_RS422_TASK_SLAVE ? "slave" : "none"));
        }
      }
    } else if(!strncmp(argv[0], "port", 4)) {
      if(argc >= 3) {
        if(!strcmp(argv[0], "porta")) {
          command = SV_OPTION_RS422A_PINOUT;
        } else if(!strcmp(argv[0], "portb")) {
          command = SV_OPTION_RS422B_PINOUT;
        } else if(!strcmp(argv[0], "portc")) {
          command = SV_OPTION_RS422C_PINOUT;
        } else if(!strcmp(argv[0], "portd")) {
          command = SV_OPTION_RS422D_PINOUT;
        } else {
          error = TRUE;
        }
  ...
  if(strncmp(what, "tc", 5) == 0) {
    ...
    switch(type) {
     ...
     case SV_DEVTYPE_PRONTOSERVER:
      ...
      break;
     default:
      sv_query(sv, SV_QUERY_OVERLAY, 0, &i);
      printf("OVERLAY       %d\n", i ? 1 : 0);
      break;
    }
    res = sv_query(sv, SV_QUERY_OVERLAYMODE, 0, &i);
    if(res == SV_OK) {
      if((i >= 0) && (i < arraysize(str_overlay))) {
        printf("OVERLAYMODE   %s\n", str_overlay[i]);
      }
    }
    res = sv_query(sv, SV_QUERY_OVERLAYTYPE, 0, &i);
    if(res == SV_OK) {
      switch(type) {
       case SV_DEVTYPE_PRONTOVIDEO:
       case SV_DEVTYPE_PRONTOVIDEO_RGB:
       case SV_DEVTYPE_PRONTOVIDEO_PICO:
        if((i >= 0) && (i < arraysize(str_overlay_type_pv))) {
          printf("OVERLAYTYPE   %s\n", str_overlay_type_pv[i]);
        }
        break;
      }
    }
...
void jpeg_debugprint(sv_handle * sv, int infinite)
{
  ...
  int  res = SV_OK;
#if defined(linux)
  int  fd = open("/proc/dvsdriver/debug", O_RDONLY);
...
int jpeg_disksetup(sv_handle * sv, char * setup, char * colormode, int compression)
{
  int  res = SV_OK;
  int  disksetup;
  int  mode = -1;

  if(!strcmp(setup, "?") || !strcmp(setup, "help")) {
    printf("sv disksetup help\n");
    jpeg_guiinfo(sv, "mode");
    return SV_ERROR_PARAMETER; /* If SV_OK prints message */
  } 

  if((strcmp("MONO", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_MONO;
  } else if((strcmp("YUV422STEREO", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_YUV422STEREO;
  } else if((strcmp("YUV422A", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_YUV422A;
  } else if(strcmp("YUV422", colormode) == 0) {
    disksetup = SV_OPTION_DISKSETUP_YUV422;
  } else if((strcmp("RGBA", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_RGBA;
  } else if((strcmp("ARGB", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_ARGB;
  } else if((strcmp("ABGR", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_ABGR;
  } else if((strcmp("RGB", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_RGB;
  } else if((strcmp("RGBAVIDEO", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_RGBAVIDEO;
  } else if((strcmp("RGBVIDEO", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_RGBVIDEO;
  } else if((strcmp("YUV420", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_YUV420;
  } else if((strcmp("YUV444", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_YUV444;
  } else if((strcmp("YUV444A", colormode) == 0) && (!compression)) {
    disksetup = SV_OPTION_DISKSETUP_YUV444A;
  } else if((strcmp("?", colormode) == 0) || (strcmp("help", colormode) == 0)) {
    printf("sv disksetup %s help\n", setup);
    printf("\tYUV422\n");
    printf("\tYUV422A\n");
    printf("\tYUV422STEREO\n");
    printf("\tYUV444\n");
    ...
    jpeg_errorprintf(sv, "sv disksetup: Unknown color mode: %s\n", colormode);
    return SV_ERROR_PARAMETER;
  }

  mode = sv_support_string2videomode(setup, 0);
 
  if(mode >= 0) {
    if(res == SV_OK) {
      res = sv_option(sv, disksetup, mode);
    }
  } else {
    jpeg_errorprintf(sv, "sv disksetup: Unknown mode: %s\n", setup);
    return SV_ERROR_PARAMETER;
  }
    ...
    if(       strcmp(argv[0], "mode") == 0) {
      if(argc >= 2) {
        if(!strcmp(argv[1], "default")) {
          ...
          videomode = sv_support_string2videomode(argv[1], 0);
        } 
      } else {
        videomode = -1;
      }
      res = sv_option_set(sv, SV_OPTION_DVI_VIDEOMODE, videomode);
    } else if(strcmp(argv[0], "info") == 0) {
      res = sv_option_get(sv, SV_OPTION_DVI_VIDEOMODE, &videomode);
      if(res == SV_OK) {
        if(videomode == -1) {
          printf("sv dvi mode default\n");
        } else {
          res = jpeg_support_videomode2string(sv, buffer, videomode);
          if(res == SV_OK) {
...
void jpeg_outduringrec(sv_handle * sv, int argc, char ** argv)
{
  int  res    = SV_OK;
  int  ok     = TRUE;
  int  value  = 0;
  ...
  if(argc == 2) {
    if(!strcmp(argv[1], "default")) { 
      value = SV_OUTDURINGREC_DEFAULT;
    } else if(!strcmp(argv[1], "input")) { 
      value = SV_OUTDURINGREC_INPUT;
    } else if(!strcmp(argv[1], "output")) { 
      value = SV_OUTDURINGREC_OUTPUT;
    } else if(!strcmp(argv[1], "black")) { 
      value = SV_OUTDURINGREC_BLACK;
    } else if(!strcmp(argv[1], "bypass")) { 
      value = SV_OUTDURINGREC_BYPASS;
    } else if(!strcmp(argv[1], "colorbar")) { 
      value = SV_OUTDURINGREC_COLORBAR;
    } else if(!strcmp(argv[1], "info")) { 
      res = sv_query(sv, SV_QUERY_OUTDURINGREC, 0, &value);
      if(res == SV_OK) {
        if((value >= 0) && (value < arraysize(str_outduringrec))) {
          printf("sv outduringrec %s\n", str_outduringrec[value]);
        } else {
          printf("sv outduringrec unknown=%d\n", value);
        }
        return;
      }
    } else if((strcmp(argv[1], "?") == 0) || (strcmp(argv[1], "help") == 0)) {
      printf("sv %s help\n", argv[0]);
      printf("\tdefault\tDefault behaviour\n");
      printf("\tblack\tShows black output on record\n");
      printf("\tinput\tShows incomming video on record\n");
      printf("\toutput\tShows output frame on record\n");
      printf("\tbypass\tShows direct bypass on record\n");
      printf("\tcolorbar\tShows colorbar on record\n");
      return;
    } else {
      ok = FALSE;
    }
  ...
  if(!ok) {
    jpeg_errorprintf(sv, "sv outduringrec: Unknown command: %s\n", argv[0]);
    return;
  }

  if(res == SV_OK) {
    res = sv_option(sv, SV_OPTION_OUTDURINGREC, value);
  } 
...
void jpeg_jack(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 2) {
    if(!strcmp(argv[1], "help") || !strcmp(argv[1], "?")) {
      printf("sv jack help\n");
      printf("sv jack list\n");
      printf("sv jack #jack assign #channel\n");
      printf("                     {out|in|out2|in2|disconnect}\n");
      printf("sv jack #jack assign ltc #\n");
      printf("                     {info}\n");
  ...
  if((res == SV_OK) && (argc >= 3)) {
    if(!strcmp(argv[2], "assign") && (argc >= 4)) {
      ...
      if(!strcmp(argv[3], "out")) {
        value = SV_JACK_CHANNEL_OUT;
      } else if(!strcmp(argv[3], "in")) {
        value = SV_JACK_CHANNEL_IN;
      } else if(!strcmp(argv[3], "out2")) {
        value = SV_JACK_CHANNEL_OUTB;
      } else if(!strcmp(argv[3], "in2")) {
        value = SV_JACK_CHANNEL_INB;
      } else if(!strncmp(argv[3], "disconnect", strlen(argv[3]))) {
        value = SV_JACK_CHANNEL_DISCONNECTED;
      } else if(!strcmp(argv[3], "ltc")) {
  ...
  if(res == SV_OK) {
    if(cmd == -3) {
      res = sv_jack_assign(sv, jack, value);
    } else if(cmd == -2) {
...
void jpeg_multichannel(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 2) {
    if(!strcmp(argv[1], "off")) { 
      ...
      res = sv_option_set(sv, SV_OPTION_MULTICHANNEL, SV_MULTICHANNEL_OUTPUT);
      ...
      if(res == SV_OK) {
        if(value == SV_MULTICHANNEL_ON) {
          res = sv_option_get(sv, SV_OPTION_MULTICHANNEL_LOCK, &value);
          if(value) {
            printf("sv multichannel lock\n");
          } else {
            printf("sv multichannel on\n");
          }
        } else if(value == SV_MULTICHANNEL_INPUT) {
          printf("sv multichannel input\n");
        } else if(value == SV_MULTICHANNEL_OUTPUT) {
          printf("sv multichannel output\n");
        } else {
      ...
      printf("sv %s help\n", argv[0]);
      printf("\tlock\tEnable multichannel (locked timebase)\n");
      printf("\ton\tEnable multichannel\n");
      printf("\toff\tDisable multichannel\n");
      printf("\tinput\tEnable multichannel for input only\n");
      printf("\toutput\tEnable multichannel for output only\n");
      return;
    } else {
      ok = FALSE;
    }
    ...
    if(lock && (res == SV_OK)) {
      res = sv_option_set(sv, SV_OPTION_MULTICHANNEL_LOCK, 1);
    }
  } else {
    ok = FALSE;
  }
