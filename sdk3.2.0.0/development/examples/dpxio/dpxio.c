...
//
//    dpxio - hows the use of the FIFO API to display and record video from/to DPX
...
/**
//  \defgroup oemdpxio Example - dpxio
//
//  This chapter shows the use of the FIFO API to display and record video from/to DPX
//  files and audio from/to an AIFF file.
//  @{
*/
...
int dpxio_exec(dpxio_handle * dpxio, int framecount)
{
  ...
  //  set the size for the captured frame. For this  we start by querying the hardware with the function \e sv_status().
  //  
  ...
  */
  ...
  if(dpxio->binput) {
    /**
    //  For the input we use the size from \e sv_status(), for the output the size of the 
  ...
  if(!dpxio->binput && !dpxio->io.dryrun) {
  ...
  if(dpxio->pulldown) {
    ...
    */
    ...
    if(dpxio->pulldownphase == SV_PULLDOWN_STARTPHASE_AUTO) {
      res = sv_live(dpxio->sv);
      if(res != SV_OK) {
        printf("ERROR: sv_live() failed = %d '%s'\n", res, sv_geterrortext(res));
        running = FALSE;
      }
      // Wait for pulldown-phase detection
      sv_usleep(dpxio->sv, 1000000);
    }
  ...
  if(running) {
    if(dpxio->baudio) {
      if(dpxio->binput) {
        ...
        if(dpxio_audio_create(&dpxio->audio, dpxio->audioio.filename, audioformat, 48000, 2, 16, (fifoflags & SV_FIFO_FLAG_AUDIOINTERLEAVED)?16:2) == AUDIO_FORMAT_INVALID) {
          printf("ERROR: Could not create audiofile\n");
          running = FALSE;
        }
      } else {
        if(dpxio_audio_open(&dpxio->audio, dpxio->audioio.filename, (fifoflags & SV_FIFO_FLAG_AUDIOINTERLEAVED)?16:2, NULL, NULL) == AUDIO_FORMAT_INVALID) {
  ...
  if(running) {
    ...
    //  For the input FIFO we might have set the FIFO flagbase as the record is done before
  ...
  */
  ...
  if(dpxio->config.vtrcontrol) {
    /**
    //  \n<b>VTR Control</b>
    //  <br>If we are doing a VTR transfer we restore the edit settings to their values.
    */
    ...
    //  call with the \e init parameter set should be done with the VTR timecode and the number 
    //  of frames that should be edited. This call will instruct the driver to preroll the VTR 
    //  and commence a play once the VTR has reached the preroll point. When the VTR is 0.5&nbsp;seconds before 
    //  the inpoint, the VTR will be up to speed (if the preroll time was enough) and the function
    //  will return the tick when the edit should commence. This can be fed into the
    //  \e sv_fifo_startex() function to start the FIFO with a timed operation which will start the input or
    //  output at the correct position.
    //
    //  This function is mostly needed for edits on the VTR. An edit from the VTR can also be done
    //  with this function or just by scanning the appropriate fields in the FIFO \e pbuffer structure
  ...
  for(frame = 0; (running && (frame < framecount)); frame++) {
    ...
    if(!bstarted) {
      if(dpxio->config.vtrcontrol && (frame > 0) && (info.tick >= when_org)) {
        ...
        //  \endverbatim
        */
        printf("dpxio: Prebuffering took to long (possibly caused by slow disks).\n");
        running = FALSE;
      } else if((frame >= 4) || dpxio->config.nopreroll) {
    ...
    //  The function <i>%sv_fifo_getbuffer()</i> returns a buffer structure that can be filled in by the user and
    ...
    */
    ...
    /** 
    //  For pulldown and rp215 we give an example on how to use the \e sv_fifo_anc() function.
    //  For non-pulldown and rp215 we give an example on how to use the \e sv_fifo_ancdata() function.
    */
    if(!dpxio->binput) {
      if(dpxio->config.rp215 || dpxio->config.rp215a) {
        if(dpxio->pulldown) {
    ...
    if(dpxio->binput) {
      if(dpxio->config.rp215 || dpxio->config.rp215a) {
        ...
        while(res == SV_OK) {
          ...
          if(res == SV_OK) {
            if((ancbuffer.did == 0x51) && (ancbuffer.sdid == 0x01)) {
              rp215_dump(ancbuffer.data, ancbuffer.datasize);
            }
          }
        }
      }
    ...
    */
    if(!dpxio->binput) {
      ...
      if(dpxio->bvideo && running) {
        vsize = dpxio_video_readframe(dpxio, frame, pbuffer, videobuffer[0], DPXIO_BUFFERVIDEOSIZE, &offset, &xsize, &ysize, &dpxtype, &nbits);
        if(!vsize) {
      ...
      if((frame == 0) && !dpxio->io.dryrun) {
        ...
        //  in format from one vertical sync to the next without any other reinitialization. One example when this is 
        //  useful is the play-out of file system files that might change in format. Please note that this is 
        //  not supported by <code><I>dpxio</I></code>. To see another example for this it is suggested to take a look at <code><I>cmodtst</I></code>.
        //
        //  Some fields in the \e sv_fifo_buffer structure need to be set and the flag
        //  \c #SV_FIFO_FLAG_STORAGEMODE needs to be set in the \e sv_fifo_getbuffer() call. In the 
        //  \e sv_fifo_putbuffer() call the needed storage parameters have to be set.
        //
        //  \verbatim
        //  struct {
        //      ...
        //    struct {
        //      int storagemode;      ///< Image data storagemode
        ...
        //      int dataoffset;       ///< Offset to first pixel in buffer
        //      int lineoffset;       ///< Offset from line to line, or 0 for default.
        //    } storage;
        //      ...
        //  } sv_fifo_buffer;
        //  \endverbatim
      ...
      if(dpxio->baudio && running) {
        running = dpxio_audio_read(&dpxio->audio, frame, dpxio->audiochannel, audiobuffer[0], pbuffer->audio[0].size / 2 / 4);
        if( !running ){
    ...
    if(dpxio->bvideo) {
      ...
      pbuffer->video[field].size = vsize + offset - (offset & ~0xfff);
    } else {
    ...
    // Settings the size to 0 skips the DMA transfer for that field.
    */
    pbuffer->video[!field].size = 0;
    ...
    if(running) {
      if(!dpxio->binput && dpxio->config.lut) {
        ...
        case 0:
          ...
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[0], 32768, 0, SV_FIFO_LUT_TYPE_3D);
          break;
        case 1:
          printf("3D LUT => inverted\n");
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[1], 32768, 0, SV_FIFO_LUT_TYPE_3D);
          break;
        case 2:
          printf("1D LUT (rgba) => identity\n");
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[2], 16384, 0, SV_FIFO_LUT_TYPE_1D_RGBA);
          break;
        case 3:
          printf("1D LUT (rgba) => inverted\n");
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[3], 16384, 0, SV_FIFO_LUT_TYPE_1D_RGBA);
          break;
        case 4:
          printf("1D LUT (rgb) => identity\n");
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[4], 6144, 0, SV_FIFO_LUT_TYPE_1D_RGB);
          break;
        case 5:
          printf("1D LUT (rgb) => inverted\n");
          res = sv_fifo_lut(dpxio->sv, pfifo, pbuffer, lut[5], 6144, 0, SV_FIFO_LUT_TYPE_1D_RGB);
          break;
        }
        if(res != SV_OK) {
          printf("ERROR: sv_fifo_lut(sv) failed = %d '%s'\n", res, sv_geterrortext(res));
          res = SV_OK;
        }
      }
    }
...
//          tcerror = TRUE;
  //      }
        ...
        frames++;
        if(frames >= svinfo.master.framerate) {
          frames = 0;
          seconds++;
        }
        if(seconds > 60) {
      ...
      if(dpxio->binput) {
        ...
        if(dpxio->bvideo && running) {
          ...
          dpxio_video_writeframe(dpxio, frame, pbuffer, videobuffer[0], vsize + offset, xsize, ysize, dpxtype, nbits, offset, padding, timecode);
        }
        if(dpxio->baudio && running) {
          dpxio_audio_write(&dpxio->audio, frame, dpxio->audiochannel, audiobuffer[0], pbuffer->audio[0].size / 4 / ((fifoflags & SV_FIFO_FLAG_AUDIOINTERLEAVED)?16:2));
        }
      }

      if(dpxio->verbose) {
        printf("Frame %4d buffer: %2d %2d %06d\n", frame, info.availbuffers, info.dropped, info.tick);
      }
  ...
  if(pfifo) {
    /**
    //  The \e sv_fifo_status() function returns the number of total buffers, free buffers and if any 
    //  frames where dropped.
    */
    res = sv_fifo_status(dpxio->sv, pfifo, &info);
    if(res != SV_OK) {
      printf("ERROR: sv_fifo_status() failed = %d '%s'\n", res, sv_geterrortext(res));
...
int main(int argc, char ** argv)
{
  ...
  while(!error && (argc > 2) && (argv[1][0] == '-')) {
    ...
    case 'p':
      ...
      case 'D':
        ...
        break;
      default:
        dpxio->pulldownphase = SV_PULLDOWN_STARTPHASE_AUTO;
      }
      break;
    case 'P':
  ...
  if(!error) {
    if(argc >= 5) {
      ...
      if(dpxio->bvideo) {
        if(!dpxio_sequence_verify(dpxio, dpxio->videoio.filename, dpxio->videoio.framenr, dpxio->videoio.framenr + framecount)) {
  ...
  if(!error) {
    res = sv_openex(&dpxio->sv, "", SV_OPENPROGRAM_DEMOPROGRAM, SV_OPENTYPE_DEFAULT, 0, 0);
    if(res != SV_OK) {
    ...
    if(!error) {
      res = sv_query(dpxio->sv, SV_QUERY_MODE_CURRENT, 0, &tmp);
      if(res != SV_OK) {
      ...
      res = sv_query(dpxio->sv, SV_QUERY_AUDIOCHANNELS, 0, &dpxio->hw.audiochannels);
      if(res != SV_OK) {
      ...
      if(dpxio->binput) {
        ...
        if(tmp & SV_MODE_STORAGE_BOTTOM2TOP) {
          printf("ERROR: Dpxio does not work in bottom2top mode\n");
          noprint = error = TRUE;
        }
        if(dpxio->config.fieldbased) {
  ...
  if(error && !noprint) { 
    printf("dpxio [options] {rec,dis} dpxfiles audiofile #start #nframes\n");
    printf("                     For audio the start is in seconds of the clip\n");
    printf("        rec,dis    - Records and Displays from video card.\n");
    printf("        filenames  - the special name 'dummy' omits disk i/o .\n");
    printf(" options:\n");
    ...
    printf("         -u          Verbose Timeing / LTC / VTR. (record)\n");
    printf("         -v          Verbose.\n");
