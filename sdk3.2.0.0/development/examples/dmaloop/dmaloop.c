...
int loop_init(loop_handle * hd)
{
  ...
  // Open first card.
  hd->svsrc = sv_open("PCI,card:0");
  if (hd->svsrc == NULL) {
    printf("loop_init: sv_open(\"PCI,card:0\") failed\n");
    loop_exit(hd);
    return FALSE;
  }
  ...
  res = sv_storage_status(hd->svsrc, 0, NULL, &storage, sizeof(storage), 0);
  if(res != SV_OK) {
    printf("loop_init: sv_storage_status() failed = %d '%s'\n", res, sv_geterrortext(res));
  }

  // How many ticks does a frame last?
  hd->vinterlace = storage.vinterlace;

  //frame to field correction
  hd->ndelay = hd->ndelay * storage.vinterlace;
...
void fill_buffer(loop_handle * hd, int binput, sv_fifo_buffer * pbuffer, int bufferid)
{
  ...
  */
  if(bufferid < 0) {
    if(binput) {
      ...
      // there is no data to play out
      buffer = (uintptr) hd->nobuffer;
      ancbuffer = (uintptr) NULL;
    }
  } else {
  ...
  */
  if(binput) {
    ...
    hd->audiosize[0] = hd->bufsrc->audio[0].size;
    hd->audiosize[1] = hd->bufsrc->audio[1].size;
  } else {
    hd->bufdst->video[0].size = hd->videosize[0];
    hd->bufdst->video[1].size = hd->videosize[1];
    hd->bufdst->audio[0].size = hd->audiosize[0];
    hd->bufdst->audio[1].size = hd->audiosize[1];
  }
...
void * loop_in(void * arg) 
{
  ...
  */
  ...
  dvs_thread_exit(TRUE, &hd->finish);
...
void * loop_out(void * arg) 
{
  ...
  */
  ...
  while (hd->running) {
    ...
    while(sv_fifo_sanitycheck(hd->svdst, hd->fifodst) == SV_ERROR_SYNC_MISSING &&
               hd->running)
    {
      if(fifo_running) {
        fifo_running = FALSE;
