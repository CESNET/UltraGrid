...
static ssize_t dvbm_q3ino_show_uid (struct class_device *cd,
	char *buf);
...
static int dvbm_q3ino_txopen (struct inode *inode, struct file *filp);
static long dvbm_q3ino_txunlocked_ioctl (struct file *filp,
		unsigned int cmd,
		unsigned long arg);
static int dvbm_q3ino_txioctl (struct inode *inode,
	struct file *filp,
	unsigned int cmd,
	unsigned long arg);
static int dvbm_q3ino_txfsync (struct file *filp,
	...
	int datasync);
static int dvbm_q3ino_txrelease (struct inode *inode, struct file *filp);
...
static int dvbm_q3ino_rxopen (struct inode *inode, struct file *filp);
static long dvbm_q3ino_rxunlocked_ioctl (struct file *filp,
		unsigned int cmd,
		unsigned long arg);
static int dvbm_q3ino_rxioctl (struct inode *inode,
	struct file *filp,
	unsigned int cmd,
	unsigned long arg);
static int dvbm_q3ino_rxfsync (struct file *filp,
	struct dentry *dentry,
	int datasync);
static int dvbm_q3ino_rxrelease (struct inode *inode, struct file *filp);

struct file_operations dvbm_q3ino_txfops = {
	.owner = THIS_MODULE,
	.llseek = no_llseek,
	.write = masterlsdma_write,
	.poll = masterlsdma_txpoll,
	.ioctl = dvbm_q3ino_txioctl,
...
#endif
	.open = dvbm_q3ino_txopen,
	.release = dvbm_q3ino_txrelease,
	.fsync = dvbm_q3ino_txfsync,
	.fasync = NULL
};

struct file_operations dvbm_q3ino_rxfops = {
	.owner = THIS_MODULE,
	.llseek = no_llseek,
	.read = masterlsdma_read,
	.poll = masterlsdma_rxpoll,
	.ioctl = dvbm_q3ino_rxioctl,
#ifdef  HAVE_UNLOCKED_IOCTL
	...
	.compat_ioctl = asi_compat_ioctl,
#endif
	.open = dvbm_q3ino_rxopen,
	.release = dvbm_q3ino_rxrelease,
 ...
 **/
static ssize_t
dvbm_q3ino_show_uid (struct class_device *cd,
	char *buf)
{
...
int __devinit
dvbm_q3ino_pci_probe (struct pci_dev *dev)
{
	int err, i;
 ...
 **/
void
dvbm_q3ino_pci_remove (struct master_dev *card)
{
	int i;
...
/**
 * dvbm_q3ino_txopen - DVB Master Quad-3in1out transmitter open() method
 * @inode: inode
 * @filp: file
 *
 * Returns a negative error code on failure and 0 on success.
 **/
static int
dvbm_q3ino_txopen (struct inode *inode, struct file *filp)
{
	...
	unsigned long arg)
{
	...
	case ASI_IOC_TXGETTXD:
		...
		if (put_user ((readl (card->core.addr + DVBM_Q3INO_ICSR(channel)) &
			DVBM_Q3INO_ICSR_TXD) ? 1 : 0, (int *)arg)) {
			return -EFAULT;
		}
		break;
	default:
...
/**
 * dvbm_q3ino_txioctl- DVB Master Quad-3in1out transmitter ioctl() method
 * @inode: inode
 * @filp: file
 * @cmd: ioctl command
 * @arg: ioctl argument
 ...
 **/
static int
dvbm_q3ino_txioctl (struct inode *inode,
	struct file *filp,
	unsigned int cmd,
	unsigned long arg)
{
	return dvbm_q3ino_txunlocked_ioctl (filp, cmd, arg);
}
...
/**
 * dvbm_q3ino_txrelease - DVB Master Quad-3in1out transmitter release() method
 * @inode: inode
 * @filp: file
 *
 * Returns a negative error code on failure and 0 on success.
 **/
static int
dvbm_q3ino_txrelease (struct inode *inode, struct file *filp)
{
	struct master_iface *iface = filp->private_data;
...
dvbm_q3ino_rxstart (struct master_iface *iface)
{
	...
	writel (reg | DVBM_Q3INO_RCSR_RXE, card->core.addr + DVBM_Q3INO_RCSR(channel));
	spin_unlock (&card->reg_lock);
...
/**
 * dvbm_q3ino_rxopen - DVB Master Quad-3in1out receiver open() method
 * @inode: inode
 * @filp: file
 *
 * Returns a negative error code on failure and 0 on success.
 **/
static int
dvbm_q3ino_rxopen (struct inode *inode, struct file *filp)
{
	return masterlsdma_open (inode,
		filp,
		dvbm_q3ino_rxinit,
	...
	unsigned long arg)
{
	...
	case ASI_IOC_RXGETCARRIER:
		/* Atomic read of ICSR, so we don't need to lock */
		if (put_user ((readl (card->core.addr + DVBM_Q3INO_ICSR(channel)) &
			DVBM_Q3INO_ICSR_RXCD) ? 1 : 0, (int *)arg)) {
			return -EFAULT;
		}
		break;
	...
	case ASI_IOC_RXGETRXD:
		/* Atomic read of ICSR, so we don't need to lock */
		if (put_user ((readl (card->core.addr + DVBM_Q3INO_ICSR(channel)) &
			DVBM_Q3INO_ICSR_RXD) ? 1 : 0, (int *)arg)) {
			return -EFAULT;
		}
		break;
...
/**
 * dvbm_q3ino_rxioctl- DVB Master Quad-3in1out Receiver ioctl() method
 * @inode: inode
 * @filp: file
 * @cmd: ioctl command
 * @arg: ioctl argument
 ...
 **/
static int
dvbm_q3ino_rxioctl (struct inode *inode,
	struct file *filp,
	unsigned int cmd,
	unsigned long arg)
{
	return dvbm_q3ino_rxunlocked_ioctl (filp, cmd, arg);
}
...
/**
 * dvbm_q3ino_rxrelease - DVB Master Quad-3in1out receiver release() method
 * @inode: inode
 * @filp: file
 *
 * Returns a negative error code on failure and 0 on success.
 **/
static int
dvbm_q3ino_rxrelease (struct inode *inode, struct file *filp)
{
	struct master_iface *iface = filp->private_data;
